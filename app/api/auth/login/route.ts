...
export async function POST(request: Request) {
  ...
  if (!rateLimit.allowed) {
    return NextResponse.json(
      { message: t("Too many login attempts. Please try again later.", "Çok fazla giriş denemesi yapıldı. Biraz sonra tekrar dene.") },
  ...
  if (!email || email.length > 254 || !password || Buffer.byteLength(password, "utf8") > 72) {
    return NextResponse.json({ message: t("Email or password is not valid.", "E-posta veya şifre uygun değil.") }, { status: 400 });
  }
  ...
  if (!user || !passwordMatches || !db.prepare(
    ...
    return NextResponse.json({ message: t("Incorrect email or password.", "E-posta veya şifre hatalı.") }, { status: 401 });
  }
